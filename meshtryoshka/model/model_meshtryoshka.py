...
class ModelMeshtryoshka(Model):
    ...
    def get_viser_render_modes(self) -> tuple[str, ...]:
        render_modes = self.get_render_modes()
        random_modes = [
    ...
    def render(
        ...
        # Choose the scene and the number of shells based on the rendering mode.
        if mode == "layered":
        ...
        elif mode == "filter_grid":
            # vertices = self.tesseract_filter.get_mesh()
            vertices = self.tesseract.get_mesh()
            vertices = self.contraction.uncontract(vertices)
            scene = vertices_to_scene(vertices)
        elif mode == "surface":
            ...
            scene = replace(
                scene,
                spherical_harmonics=torch.zeros_like(scene.spherical_harmonics),
