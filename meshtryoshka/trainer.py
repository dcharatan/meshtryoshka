...
class Trainer:
    ...
    def load_checkpoint(self, path: Path) -> None:
        ...
        self.model.load_step(self.step)
    ...
    def train(self) -> None:
        ...
        def train_work_fn() -> bool:
            ...
            if self.step % self.cfg.train.visualization_interval == 0:
                with torch.no_grad():
                    self.visualize(self.val_dataset, "val")
                    self.visualize(self.train_dataset, "train")

            if self.step % self.cfg.train.dump_images_interval == 0:
                with torch.no_grad():
                    self.dump_images(self.test_dataset, "test", self.cfg.train.num_dump)
            ...
            if self.step % self.cfg.train.val_interval == 0:
                with torch.no_grad():
                    self.log_renders(self.test_dataset, "test")
            if self.step == self.cfg.train.num_steps:
                with torch.no_grad():
                    self.log_renders(self.test_dataset, "test")
        ...
        if self.cfg.eval_only or self.cfg.render_trajectory_only:
            if self.cfg.eval_only:
                self.log_renders(self.test_dataset, "test")

            if self.cfg.render_trajectory_only:
            ...
            viewer = Viewer(
                render_fn,
                self.model.get_viser_render_modes(),
                self.train_dataset,
        ...
        if not self.cfg.eval_only and not self.cfg.render_trajectory_only:
    ...
    def benchmark_rendering_speed(
        ...
        dump_path = self.log_dir / "benchmark_layered_images"
        ...
        dump_path = self.log_dir / "benchmark_surface_images"
        dump_path.mkdir(exist_ok=True, parents=True)
        ...
        dump_path = self.log_dir / "rendering_benchmark.json"
    ...
    def visualize(self, dataset: LoadedDataset, stage) -> None:
        ...
        images = {
            ...
            for mode in self.model.get_render_modes()
        }
    ...
    def render_trajectory(self, dataset: LoadedDataset, tag: str) -> None:
        ...
        output_path = os.path.join(self.log_dir, f"{tag}_trajectory_{self.step}")
        os.makedirs(output_path, exist_ok=True)
        for key, video in videos.items():
            write_lossless_video(video, f"{output_path}/{key}.mp4", fps=24)

    def log_renders(
        self, dataset: LoadedDataset, tag: str, compute_metrics=True
    ) -> None:
        logging.info(f"Logging Rendered Images at step {self.step}.")

        log_path = os.path.join(self.log_dir, f"{tag}_render_{self.step}")
        os.makedirs(log_path, exist_ok=True)
        all_metrics, individual_metrics = self.model.metrics(
            dataset,
            compute_metrics,
        ...
        if compute_metrics:
            ...
            for render_type, metrics in all_metrics.items():
                output_path = os.path.join(
                    self.log_dir, f"{tag}_render_{self.step}", f"{render_type}.json"
                )
                with open(output_path, "w") as json_file:
                    json.dump(metrics, json_file, indent=4)

            for render_type, metrics in individual_metrics.items():
                output_path = os.path.join(
                    self.log_dir,
                    f"{tag}_render_{self.step}",
                    f"individual_{render_type}.json",
                )
                with open(output_path, "w") as json_file:
                    json.dump(metrics, json_file, indent=4)

    def dump_images(self, dataset: LoadedDataset, tag: str, num_dump):
        log_path = os.path.join(self.log_dir, f"{tag}_render_{self.step}")
        os.makedirs(log_path, exist_ok=True)
        self.model.dump_images(
            dataset,
            num_dump=num_dump,
            output_path=log_path,
        )
