...
def render_trajectory(
    ...
    frames_between_keyframes: int = 30,
    keyframe_limit: int = 5,
    batch_size: int = 15,
) -> dict[str, Float[Tensor, "time rgb=3 height width"]]:
    device = dataset.extrinsics.device

    # Limit the number of rendered frames.
    extrinsics = dataset.extrinsics[:keyframe_limit]
    intrinsics = dataset.intrinsics[:keyframe_limit]
    b, _, _ = extrinsics.shape

    # Interpolate the dataset's cameras sequentially.
    t = torch.linspace(
        0,
        1,
    ...
    extrinsics = [
        interpolate_extrinsics(extrinsics[i - 1], extrinsics[i], t) for i in range(b)
    ]
    extrinsics = torch.cat(extrinsics, dim=0)
    intrinsics = [
        interpolate_intrinsics(intrinsics[i - 1], intrinsics[i], t) for i in range(b)
    ]
    ...
    # Render in batches
    rendered_outputs = {mode: [] for mode in model.get_eval_render_modes()}
    num_frames = extrinsics.shape[0]

    for start in range(0, num_frames, batch_size):
        end = min(start + batch_size, num_frames)
        batch_extrinsics = extrinsics[start:end]
        batch_intrinsics = intrinsics[start:end]
        for mode in rendered_outputs:
            output = model.render(batch_extrinsics, batch_intrinsics, (h, w), mode=mode)
            rendered_outputs[mode].append(output)

    # Concatenate all batches along time (batch) dimension
    # TODO: OOM here???
    return {mode: torch.cat(frames, dim=0) for mode, frames in rendered_outputs.items()}
